...
define('INVOICEFROMUID2', 'SELECT ord.orderId, ord.orderCode, ord.totalprice, ord.status, ord.orderDate, cstm.uid FROM `order` AS ord INNER JOIN customer AS cstm ON(ord.customerId=cstm.customerId) WHERE cstm.uid=?');
define('CUSTOMERINFO', 'SELECT cstm.customerId, cstm.firstName, cstm.lastName, cstm.email, cstm.phone, addr.street, addr.postalCode, addr.city FROM customer AS cstm INNER JOIN address AS addr ON(cstm.addressId=addr.addressId) WHERE cstm.uid=?');
define('INVOICEFROMORDERID', 'SELECT ord.orderId, ord.orderCode, ord.totalprice, ord.status, ord.orderDate FROM `order` AS ord INNER JOIN customer AS cstm ON(ord.customerId=cstm.customerId) WHERE ord.orderId=? and cstm.uid=?');
